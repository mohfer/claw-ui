...
export function ChatInput({ onSend, isStreaming, onCancel }) {
  ...
  useEffect(() => {
    if (value === "" && !isStreaming && textareaRef.current) {
    ...
    <footer className="px-6 pt-4 pb-6 border-t border-[#e8e6e0] bg-[#f7f6f3] shrink-0">
      <div className="flex items-end gap-2.5 bg-white border border-[#e8e6e0] rounded-2xl px-4 py-2.5 pr-2.5 transition-colors focus-within:border-[#9a9690]">
        ...
        {!isStreaming ? (
          ...
          <button
            className="w-[34px] h-[34px] rounded-[10px] bg-[#d4522a] text-white border-none cursor-pointer flex items-center justify-center shrink-0 transition-colors hover:bg-[#b33e20] disabled:opacity-30 disabled:cursor-not-allowed"
            onClick={onCancel}
            aria-label="Cancel"
          >
            <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
